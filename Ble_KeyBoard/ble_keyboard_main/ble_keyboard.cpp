...
class BLEKeyboard {
public:
  BLEKeyboard() : server(nullptr), hid(nullptr), input(nullptr), output(nullptr), advertising(nullptr) {}
  ...
  void begin() {
    ...
    const uint8_t reportMap[] = {
      ...
      REPORT_SIZE(1), 0x01,
      REPORT_COUNT(1), 0x08,
      0x81, 0x02,            // Data, Var, Abs
      REPORT_COUNT(1), 0x01,
      REPORT_SIZE(1), 0x08,
      0x81, 0x01,            // Const, Array, Abs
      REPORT_COUNT(1), 0x05,
      REPORT_SIZE(1), 0x01,
  ...
  void sendKey(uint8_t keycode) {
    ...
    input->notify();
    delay(10);  // Reduced delay for better performance
